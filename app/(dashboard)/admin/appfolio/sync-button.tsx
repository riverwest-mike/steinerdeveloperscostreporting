    ...
    <div className="space-y-4">
      <div className="flex items-end gap-3">
        <div className="space-y-1">
          <label className="text-xs font-medium" htmlFor="sync-from">From</label>
          <input
            id="sync-from"
            type="date"
            value={fromDate}
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium" htmlFor="sync-to">To</label>
          <input
            id="sync-to"
            type="date"
            value={toDate}
      ...
      {result && (
        <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3 dark:border-green-800 dark:bg-green-950/30">
          ...
          <div className="mt-2 grid grid-cols-3 gap-4 text-xs text-green-700 dark:text-green-300">
            <div>
              <span className="font-medium">{result.records_fetched?.toLocaleString() ?? 0}</span> records fetched
            </div>
            <div>
